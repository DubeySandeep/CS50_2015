...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            char* qind = strchrnul(requestline[REQUEST_TARGET], '?');
            char* eind = strchr(requestline[REQUEST_TARGET], '\0');
            char query[(eind - qind < 1) ? 1 : eind - qind];
            memcpy(
                query, 
                (strncmp(qind, "\0", 1) == 0) ? qind : qind + 1, 
                (eind - qind < 1) ? 1 : eind - qind
            );
            
            // get the absolute-path
            qind = strchrnul(requestline[REQUEST_TARGET], '?');
            char absolutePath[qind - requestline[REQUEST_TARGET] + 1];
            strncpy(
                absolutePath, 
                requestline[REQUEST_TARGET], 
                qind - requestline[REQUEST_TARGET]
            );
            absolutePath[qind - requestline[REQUEST_TARGET]] = '\0';
            ...
            eind = strchr(absolutePath, '\0');
            char extension[eind - extInd];
            memcpy(extension, extInd + 1, eind - extInd);
